    ...
    <div className="my-20 relative">
      <div className="flex justify-between my-5">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex">
            {Array.from({ length: flashSaleProducts.length }).map(
              (item, index) => (
                <div
                  key={index}
                  className={`h-2 mx-1 w-2 rounded-full bg-gray`}
                />
          ...
          >
            {flashSaleProducts.map((item, index) => (
              <SwiperSlide key={index}>
                <div
                  key={index}
                  className="flex flex-col w-full items-center gap-2 border border-gray rounded-b-2xl pb-5"
                >
                  <div className="w-full h-[150px] relative">
                    <span className="bg-primary rounded-lg p-2 absolute right-3 top-3">
                      25% OFF
                    </span>
                    <img
                      src={item.image}
                  ...
                  <span>
                    ...
                    - <span className="text-primary">${item.promoPrice}</span>
                  </span>
                  <div className="w-[80%]">
                    ...
                    <ProgressBar
                      bgColor="#FCC313"
                      completed={(item.sold / item.available) * (100 / 1)}
                    />
                  </div>
                </div>
              </SwiperSlide>
