...
const VendorIntroSlider = () => {
  ...
  const prevSlide = () => {
    if (slideIndex !== 1) {
      ...
      setSlideIndex(vendorIntroSlider.length);
    }
  };
  ...
  return (
    ...
    >
      {vendorIntroSlider.map((obj, index) => {
        return (
          <div
            key={obj.id}
      ...
      <div className="absolute bottom-2 left-[40%] lg:left-[45%] flex">
        {Array.from({ length: vendorIntroSlider.length }).map((item, index) => (
          <div
            key={index}
...
export default VendorIntroSlider;
